...
float light_range = light.radius * 2.0;
...
float hack_diffuse_radial_influence = (1.0 - pow(clamp(light_distance / light_range, 0.0, 1.0), 2));
float hack_specular_radial_influence = (1.0 - pow(clamp(light_distance / light_range, 0.0, 1.0), 10));
// Taper light intensity more softly to preserve light with distance
float hack_soft_tapering = (20.0 * (light_distance / light_range));

vec3 radiant_flux = light.color * light.power * light_range;
